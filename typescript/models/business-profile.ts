...
type BusinessProfileCategoryEnum = 'ACCT' | 'ART' | 'BEAUTY' | 'CATERING' | 'CHARITY' | 'CLEANING' | 'CONSULTANT' | 'CONTRACTOR' | 'DENTIST' | 'EDU' | 'FOOD' | 'LANDSCAPING' | 'LEGAL' | 'MEDICAL_PRACT' | 'MEDICAL_SERV' | 'MEMBERSHIP' | 'MISC_FOOD_STORES' | 'MISC_MERCH' | 'MISC_SERV' | 'MUSIC' | 'PC' | 'PHOTO_FILM' | 'PROF_SERV' | 'REAL_ESTATE' | 'RECREATION' | 'REPAIR' | 'RESTO' | 'RETAIL' | 'TAXI' | 'UTILITY' | 'VET' | 'WEB_DEV' | 'WEB_HOSTING' | 'OTHER'
type BusinessProfileLocaleEnum = 'en_US' | 'en_CA' | 'fr_CA' | 'en_GB'
type BusinessProfileRegionEnum = 'US' | 'CA'
type BusinessProfileStructureEnum = 'CHARITY' | 'CIC' | 'CORP' | 'LTD' | 'LLC' | 'LLP' | 'NPCORP' | 'PARTNERSHIP' | 'PLC' | 'GOV' | 'SOLEPROP' | 'TRUST'
