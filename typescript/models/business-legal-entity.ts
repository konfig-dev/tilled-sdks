...
type BusinessLegalEntityCategoryEnum = 'ACCT' | 'ART' | 'BEAUTY' | 'CATERING' | 'CHARITY' | 'CLEANING' | 'CONSULTANT' | 'CONTRACTOR' | 'DENTIST' | 'EDU' | 'FOOD' | 'LANDSCAPING' | 'LEGAL' | 'MEDICAL_PRACT' | 'MEDICAL_SERV' | 'MEMBERSHIP' | 'MISC_FOOD_STORES' | 'MISC_MERCH' | 'MISC_SERV' | 'MUSIC' | 'PC' | 'PHOTO_FILM' | 'PROF_SERV' | 'REAL_ESTATE' | 'RECREATION' | 'REPAIR' | 'RESTO' | 'RETAIL' | 'TAXI' | 'UTILITY' | 'VET' | 'WEB_DEV' | 'WEB_HOSTING' | 'OTHER'
type BusinessLegalEntityCurrencyEnum = 'aud' | 'cad' | 'dkk' | 'eur' | 'hkd' | 'jpy' | 'nzd' | 'nok' | 'gbp' | 'zar' | 'sek' | 'chf' | 'usd'
type BusinessLegalEntityLocaleEnum = 'en_US' | 'en_CA' | 'fr_CA' | 'en_GB'
type BusinessLegalEntityRegionEnum = 'US' | 'CA'
type BusinessLegalEntityTypeEnum = 'CHARITY' | 'CIC' | 'CORP' | 'LTD' | 'LLC' | 'LLP' | 'NPCORP' | 'PARTNERSHIP' | 'PLC' | 'GOV' | 'SOLEPROP' | 'TRUST'
type BusinessLegalEntityYearlyVolumeRangeEnum = 'LOW' | 'MEDIUM' | 'HIGH' | 'VERY_HIGH'
